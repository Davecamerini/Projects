<?php
//il file serve per cancellare ricorsivamente dalla gallery della sezione modifica borgo le immagini.
...
if (isset($_POST['action']) && $_POST['action'] == 'remove_image' && isset($_POST['image']) && isset($_POST['id'])) {
    ...
    $borgo_id = intval($_POST['id']);

    // Recupera la gallery attuale
    $query = "SELECT gallery FROM borghi_scheda WHERE id=$borgo_id";
    ...
    if ($result->num_rows > 0) {
        ...
        $update_query = "UPDATE borghi_scheda SET gallery='$new_gallery' WHERE id=$borgo_id";
        ...
        $response['message'] = "borgo non trovato.";
    }
}
