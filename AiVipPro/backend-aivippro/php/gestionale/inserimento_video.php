...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (file_exists($target_file)) {
        echo "Spiacenti, il file video esiste già.";
        $uploadOk = 0;
    }
    ...
    // Accetta solo determinati formati di file video
    if (
        $videoFileType != "mp4" && $videoFileType != "avi" && $videoFileType != "mov" &&
        $videoFileType != "mkv" && $videoFileType != "wmv"
    ) {
        echo "Spiacenti, sono consentiti solo file video MP4. Il file che hai provato a caricare è di tipo: $videoFileType";
        $uploadOk = 0;
    }
