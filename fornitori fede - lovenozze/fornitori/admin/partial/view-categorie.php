...
$query = "SELECT id, titolo, descrizione, data_inserimento, genitore, immagine FROM fornitori_categorie WHERE genitore = 0";
$categories = $conn->query($query);

// Funzione per ottenere le sottocategorie in base all'ID del genitore
function get_subcategories($parent_id, $conn) {
    $sub_query = "SELECT id, titolo, descrizione, data_inserimento, immagine FROM fornitori_categorie WHERE genitore = $parent_id";
...
<div class="content-card">
  <div style="display: flex; flex-direction: row; flex-wrap: nowrap; align-items: center; justify-content: space-between;">
    ...
    <a href='insert-categoria.php' class='btn btn-sm btn-success'>Nuova Categoria</a>
  </div>
    ...
    <tbody>
      ...
      if ($categories->num_rows > 0) {
        while ($category = $categories->fetch_assoc()) {
          ...
          echo "<td>
                  <a href='modifica_categoria.php?id=" . $category['id'] . "' class='btn btn-sm btn-dark'>Modifica</a>
                  <a href='elimina_categoria.php?id=" . $category['id'] . "' class='btn btn-sm btn-danger' onclick='return confirm(\"Sei sicuro di voler eliminare questa categoria?\");'>Elimina</a>
                </td>";
          echo "</tr>";
          ...
          if ($subcategories->num_rows > 0) {
            while ($subcategory = $subcategories->fetch_assoc()) {
              ...
              echo "<td>
                      <a href='modifica_categoria.php?id=" . $subcategory['id'] . "' class='btn btn-sm btn-dark'>Modifica</a>
                      <a href='elimina_categoria.php?id=" . $subcategory['id'] . "' class='btn btn-sm btn-danger' onclick='return confirm(\"Sei sicuro di voler eliminare questa sottocategoria?\");'>Elimina</a>
                    </td>";
              echo "</tr>";
            }
          }
