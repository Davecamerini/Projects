...
function vsp_video_page() {
    ...
    if ($items) {
        echo '<h2 class="vsp-title">Available Videos and Folders</h2><ul class="vsp-item-list">';
        
        // Link to go back to the parent directory
        if ($current_dir) {
            $parent_dir = dirname($current_dir);
            $parent_url = add_query_arg('dir', $parent_dir);
            echo '<li class="vsp-parent-dir"><a href="' . esc_url($parent_url) . '">.. (Go Up)</a></li>';
        }
        ...
        foreach ($items as $item) {
            if ($item !== '.' && $item !== '..') {
                ...
                if (is_dir($item_path)) {
                    ...
                    echo '<li class="vsp-folder"><a href="' . esc_url($folder_url) . '">' . esc_html($item) . '</a></li>';
                    ...
                    echo '<li class="vsp-video"><a href="' . esc_url($video_url) . '" target="_blank">' . esc_html($item) . '</a></li>';
                }
            }
        }
        echo '</ul>';
    } else {
        echo '<p class="vsp-no-items">No videos or folders found.</p>';
    }
